...
def train_xgb_regressor(X_train, y_train, X_test, y_test, use_gpu=False):
    ...
    additional_params = {}
    if use_gpu:
        print("GPU support is not available on Mac M1. Switching to CPU.")
        use_gpu = False  # Force CPU usage

    xgb_regressor = XGBRegressor(base_score=0.5, booster='gbtree',
                                 n_estimators=3000,
                                 ...
                                 reg_alpha=0,
                                 reg_lambda=1,
                                 tree_method='hist',  # Ensure hist is used for CPU
                                 predictor='cpu_predictor',  # Ensure CPU predictor is used
                                 **additional_params)
